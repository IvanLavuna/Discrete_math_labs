...
// i will use algorithm that is a lot more efficient O(m) m -edges
// max power of graph must be <= than 20
void Graph::Euler_cycle(int start) // !!!!!!!!! here +1
{

    Graph temp_graph(m_nodes);
    //copy member graph
    for(int  i=0;i<m_nodes;i++)
    {
        for(auto item : m_arr[i])
        {
            temp_graph.add_edge(i,item.V,item.weight);
        }
    }

    Eyler_cycle(start,temp_graph);


}

void Graph::Eyler_cycle(int start,Graph &graph)
{
    cout<<start<<" ";

    vector<int> dfs_list;

    graph.DFS(start,dfs_list,false);
    dfs_list.push_back(start);
    if(dfs_list.size() ==2)
    {
        return;
    }
    for(int i =0;i<dfs_list.size() - 1;i++)
    {
        graph.remove_edge(dfs_list[i],dfs_list[i+1]);
        graph.remove_edge(dfs_list[i+1],dfs_list[i]);
    }

    for(int i =0; i < dfs_list.size(); i++)
    {
        Eyler_cycle(dfs_list[i], graph);
    }
